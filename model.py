...
class model():
    ...
    def train(self, x_train, y_train, x_valid, y_valid, epochs= 100, batch_size= 10, save_path= None):
        if save_path == None:
            save_path = self.name

        weight_path = ".\\result\\model_record\\" + save_path
        ...
        history = self.model.fit(x_train, y_train, validation_data=(x_valid, y_valid), batch_size= batch_size, epochs=epochs, callbacks=[saveModel, checkBestPoint ,reduce_lr])
        self.model.save(".\\result\model_record\\" + save_path + '.h5')

        ex_loss = excel.Excel()
        ex_loss.write_loss_and_iou(save_path, history.history['loss'], history.history['val_loss'], 0, 0)
        ex_loss.save_excel(file_name=".\\result\data\\" + save_path + "_loss.xlsx")
        ex_loss.close_excel()
    ...
    def train_generator(self, dataset, train_num, valid_num, epochs=100, batch_size=10):
        ...
        history = model.fit_generator(self.myGenerator(start_num= 1, batch_size= batch_size, dataset= dataset, total_num= train_num),
        ...
        ex_loss.close_excel()
    def test(self, x_test, y_test, batch_size= 10, save_path = None):
        # if x_test.ndim == 3:
        ...
        self.postprocessing(test_y= y_test, predict_y=y_predict, save_path=save_path)

    def postprocessing(self, test_y, predict_y, save_path, threshold = 0.5):
        def mkdir(path):
            # 去除首位空格
            path = path.strip()
            # 去除尾部 \ 符號
        ...
        for index in range(len(test_y)):
            ...
            cv2.imwrite(".\\result\image\\" + save_path + '\\{}.png'.format(index + 1), img_save)
            print('Save image:{}'.format(1+index))

        ex_iou = excel.Excel()
        ex_iou.write_excel("a1", save_path)
        ex_iou.write_excel("a2", "image num")
        ex_iou.write_excel("b2", "iou")
        ex_iou.write_excel("c2", "avg. iou")
        ex_iou.write_excel("d2", "precision")
        ex_iou.write_excel("e2", "avg. precision")
        ex_iou.write_excel("f2", "recall")
        ex_iou.write_excel("g2", "avg. recall")
        ex_iou.write_excel("h2", "F1")
        ex_iou.write_excel("i2", "avg. F1")

        for index in range(len(test_y)):
            ex_iou.write_excel("a" + str(index + 3), str(index+1))
        ex_iou.write_excel("b3", iou, vertical=True)
        ex_iou.write_excel("c3", avr_iou, vertical=True)
        ex_iou.write_excel("d3", precision, vertical=True)
        ex_iou.write_excel("e3", avr_precision, vertical=True)
        ex_iou.write_excel("f3", recall, vertical=True)
        ex_iou.write_excel("g3", avr_recall, vertical=True)
        ex_iou.write_excel("h3", F1, vertical=True)
        ex_iou.write_excel("i3", avr_F1, vertical=True)
    ...
    def myGenerator(self, start_num, batch_size, dataset, total_num):
        while True:
            for i in range(total_num // batch_size):
                yield data.load_data(dataset= dataset, start_num= start_num + i * batch_size, total_num= batch_size, size= (256, 256, 4))
...
class Judgement_model(object):
    ...
    def train(self, x, y, x_valid, y_valid, epochs= 100, batch_size= 10):
        weight_path = ".\\result\\model_record\\" + self.name
        checkBestPoint = ModelCheckpoint(weight_path + '-bestweights-epoch{epoch:03d}-loss{loss:.5f}-val_loss{val_loss:.5f}.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='min', save_weights_only=True)
        saveModel = ModelCheckpoint(weight_path + '-weights-epoch{epoch:03d}-loss{loss:.5f}-val_loss{val_loss:.5f}.h5', verbose=1, save_best_only=False, save_weights_only=True)
        reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=10, mode='auto')
    ...
    def test(self, x, y, data_start, batch_size= 10, save_path = None):
        ...
        print("Postprocessing.")
        self.postprocessing(test_y= y, predict_y= predict_y, save_path= save_path)
    ...
    def postprocessing(self, test_y, predict_y, save_path, threshold = 0.5):
        ...
        for index in range(len(test_y)):
            ex_acc.write_excel("a" + str(index + 3), str(1+index))
...
class SI_mark_processing(object):
    ...
    def predict(self, x, y, judge_weights_path, pipe_weights_path, save_path,judge_threshold = 0.5):
        ...
        self.postprocessing(high_y, predict_high_y, high_save_path, threshold=0.5)
        low_save_path = save_path + "predictLowData"
        self.postprocessing(low_y, predict_low_y, low_save_path, threshold=0.5)

        # 針對最終融合進行後處理
        print("End concate predict data")
        print("Postprocessing")
        self.postprocessing(y, predict_y, save_path, threshold = 0.5)
    ...
    def postprocessing(self, test_y, predict_y, save_path, threshold = 0.5):
        ...
        for index in range(len(test_y)):
            ex_iou.write_excel("a" + str(index + 3), str(1+index))
        ex_iou.write_excel("b3", iou, vertical=True)
        ex_iou.write_excel("c3", avr_iou, vertical=True)
        ex_iou.write_excel("d3", precision, vertical=True)
        ex_iou.write_excel("e3", avr_precision, vertical=True)
